-- Fix booking status transition rules to allow workers to accept pending bookings
...
BEGIN
  ...
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    ...
    -- From accepted: can go to on_the_way or cancelled
    IF OLD.status = 'accepted' AND NEW.status NOT IN ('on_the_way', 'cancelled') THEN
      RAISE EXCEPTION 'Invalid transition from accepted to %', NEW.status;
    END IF;
    
    -- From on_the_way: can go to started or cancelled
    IF OLD.status = 'on_the_way' AND NEW.status NOT IN ('started', 'cancelled') THEN
