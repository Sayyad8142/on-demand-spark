-- Update the booking status transition trigger to allow workers to complete from assigned status
...
BEGIN
  ...
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    ...
    -- From assigned: can go to accepted, started, completed, cancelled, or back to pending
    IF OLD.status = 'assigned' AND NEW.status NOT IN ('accepted', 'started', 'completed', 'cancelled', 'pending') THEN
      RAISE EXCEPTION 'Invalid transition from assigned to %', NEW.status;
    END IF;
    
    -- From accepted: can go to on_the_way, started, completed, or cancelled
    IF OLD.status = 'accepted' AND NEW.status NOT IN ('on_the_way', 'started', 'completed', 'cancelled') THEN
