        ...
        {navItems.map(({ path, icon: Icon, label, isExternal }) => {
          ...
          if (isCallButton) {
            return (
              ...
              >
                <div
                  className="w-9 h-9 rounded-full flex items-center justify-center text-white shadow-md shadow-[#16C75A]/30 transition-all duration-150 active:scale-95 group-hover:brightness-110"
                  style={{ backgroundColor: '#16C75A' }}
                >
                  <Phone className="w-5 h-5 fill-white" strokeWidth={2.5} />
                </div>
                <span className="text-xs mt-1 font-medium text-muted-foreground">Call</span>
              </button>
            );
          }
